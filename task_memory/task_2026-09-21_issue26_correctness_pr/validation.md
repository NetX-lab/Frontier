...
| Negative control on the unrepaired source (added 2026-09-24, workflow `wf_7606e14e-f10` finding 26) | The `7269bac` tests against `7269bac^`'s `moe_vllm_kernel.py`: 1 failed, 6 errors. Every error is fixture setup (`_vllm_custom_ops` does not exist there), so no test reaches its assertion. A mutant of `7269bac` that restores only the slice arithmetic: 5 failed, 2 passed, and the discriminating test fails on its assertion. That mutant, not the unrepaired source, is the W6 negative control. Evidence: `wf_7606e14e_f10/w6_negative_controls.txt`. |
...
| Native GPU parity | **PASS. 8 passed in 13.70 s** on `NVIDIA H800` (`gpu-h800-0110`), job `exp-0922-145047-660565`, creator `i-fengyicheng`, charged group `codesign` per the user's 2026-09-22 instruction, 1 GPU, image `artifactory.stepfun-inc.com/docker-public/vllm/vllm-openai:v0.10.2`, NFS source `100.96.128.195:/data/ycfeng/Frontier/.worktrees/issue26-correctness-pr`. Worker: Python 3.12.11, torch 2.8.0+cu128, vLLM 0.10.2, `VLLM_API_VERSION=0.10.x`, `FP8_AVAILABLE=True`. Seven of the 8 cases of `tests/integration/test_moe_fused_expert_numerical_parity.py` compare at `rtol=0, atol=0` against `fused_experts`: Qwen3-A3B-30B shapes from the checked-in model config at 4096 and 4097 tokens on EP ranks 0 and 1; a 257-token, 16-expert case at top-k 2 and 4 whose routing leaves two local experts empty; and repeated invocation with different inputs. The eighth checks the FP8 path structurally: the quantizer's input shape and a finite output, with no reference comparison (count corrected 2026-09-24, workflow `wf_7606e14e-f10` finding 26). Three earlier attempts and their causes are in `test_report_2026-09-22_w6_fused_expert_arithmetic.md` section 8. |
...
| Native rerun with the corrected FP8 wiring (external review C35-03) | `exp-0922-202645-561899`, `codesign` / H800 (`gpu-h800-0095`), vLLM 0.10.2 image, worktree `c231322`: **8 passed in 14.27 s**, exit 0. Seven zero-tolerance comparisons plus the FP8 structural check, now with `block_shape` reaching both GEMMs. FP8 numerics still not compared against a reference. **Superseded 2026-09-24:** `f236c17` (W6-R1..R3) changed the FP8 step after `c231322` (kernel config lookup, compute type, in-step quantization), so no native run covered the current FP8 code until the rerun below. |
| Native rerun of the current FP8 code (2026-09-24, user decision A7) | `exp-0924-114241-429126`, `codesign` / H800 (`gpu-h800-0203`), vLLM 0.10.2 image, worktree clean at `363a1dd`: **8 passed in 14.53 s**, exit 0 (`w6_native_20260924/`). Seven zero-tolerance comparisons plus the FP8 structural check on the `f236c17` FP8 step. FP8 numerics still not compared against a reference. |
| CPU setup-argument check (added 2026-09-24, `6828581`, W6-R6) | The native test copies the profiler's tile-config and alignment calls instead of calling them, and the CPU stubs ignored their arguments, so nothing checked what `profile_fused_moe_kernel` passes. `test_the_tile_config_and_the_alignment_follow_fused_experts` asserts the arguments vLLM's `fused_experts_impl` passes: block shape, token count and top-k for `try_get_optimal_moe_config`; `BLOCK_SIZE_M`, the global expert count and the EP expert map for `moe_align_block_size`. Three single-argument mutants of the profiler (`block_shape=None`; the local expert count; `expert_map=None`) each give 1 failed, 31 passed, and only the new test fails. HEAD: 32 passed. Evidence: `wf_7606e14e_f10/w6_negative_controls.txt`. |
...
| Item | Result |
| --- | --- |
| Reachability | Confirmed by source. `moe_operator_times.py:512` computes `data_size_bytes = embedding_dim * 2 * routed_tokens` and hands it to `predict_all_to_all`, so an EP lane routing no token in a step asks for an empty transfer; `predict_reduce_scatter` floor-divides by the device count and reaches zero for any payload below it. `base_cc_backend._validate_data_size` rejects only negative sizes, so zero reaches the runner. **Corrected 2026-09-24 (workflow `wf_7606e14e-f10` S2):** no Frontier path calls `predict_reduce_scatter`, so its clause describes no reachable case. The reachable zero is the EP all-to-all. The other collectives Frontier prices (all-reduce, all-gather, send/recv) take their payload from a real batch with at least one token. Idle placeholders are never priced, and a zero MoE-TP all-reduce returns 0.0 through `ZeroPayloadPolicy.EXACT_NOOP` before the backend is called. |
...
| W7-R4 fix (added 2026-09-24, user decision option a) | Companion `e922c77` on `fix/zero-payload-input-handling`: the runner exits 2 with `tensor_bytes=0 is defined only for alltoall, not <kind>` for allreduce, allgather, reducescatter, p2p and t2g, before building any flow. Companion `tests/test_zero_payload_input.py`: 16 passed; the four new cases fail on `ff11ee6` (4 failed, 12 passed). On `ff11ee6` the same one-server scenario at payload 0 exited 0 (intra-server edges are one-byte placeholders), so only cross-server groups hung. Cross-server probe (2 x 8 GPUs, DP=16 ring) on `e922c77`: allreduce, allgather and reducescatter at payload 0 return runner exit 2 at once. Frontier gitlink `9adf759`: `tests/unit/test_collective_sim_zero_payload.py` 3 passed. No Frontier source changed and no fidelity case selects `collective_sim`, so the matrix was not rerun. Evidence: `wf_7606e14e_f10/w7_r4_zero_payload_by_kind.txt`. |
| Gitlink re-point (2026-09-24) | Companion PR 1 merged into `main` as `d28fe917` (user, 04:25:59Z); `git diff e922c77 d28fe917` is empty. Gitlink `1b11eff`. Companion `tests/test_zero_payload_input.py` 16 passed; Frontier `tests/unit/test_collective_sim_zero_payload.py` 3 passed. Fresh clone of the pushed branch: 1 skipped without the submodule; `git submodule update --init` checks out `d28fe917` from GitHub; `make -j16` exits 0; then 3 passed and 16 passed. Evidence `/data/ycfeng/tmp/issue26-correctness-pr/q9_gitlink_20260924/` (`companion.txt`, `frontier.txt`, `clean_checkout.sh`, `clean_checkout.txt`) |
...
| A2b (added 2026-09-24, workflow `wf_7606e14e-f10` S5) | `review_20260924/w904/scripts/pp_sweep.py` and `reproduce/scripts/pdd_cell.sh` on `ba0a804` exports, with the control disabling the withdrawal; one cell of each rechecked on `5e7221d` | Every control stall at PP>1 or in PDD drains with the fix | Co-location: control stalls 17 of 288 (PP 1-3, all `attn_dp=4`), fix 288 of 288 at 24 of 24, and the withdrawal fires in exactly those 17 cells. PDD: control stalls 4 of 36 (PP 1 and 2), fix 36 of 36. `5e7221d`: both recheck cells stall on the control and finish 24 of 24 with the fix (`wf_7606e14e_f10/w9_04_reach.txt`) | PASS |

A2 ran co-location at PP=1 only. A2b was added after the review showed that
the defect also reaches PP>1 and PDD (`issues.md` W9-04, "Reach beyond PP=1").
