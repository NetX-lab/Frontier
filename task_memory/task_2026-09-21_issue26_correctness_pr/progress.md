...
| 2026-09-24 | PR 35 body updated for W3, `8cfc995`, PR 38, the FP8 rerun and the calibration-tool decision. |
...
| 6 Calibration tools | in-progress: option (a) decided in round 2. Workflow `wf_147bad0f-83a` stopped after `research:spec` (saved as `salvage_20260924/research_spec.md`). Grok task G4 collected the normalizer facts and 16 open decisions (see "Grok task G4 results" below); the design waits for a round with the user | `requirements.md`, "[Decision] 2026-09-24 — calibration tools, round 2" |
...
| Rate-limited subagents | in-progress: a message cannot resume a workflow agent ("No transcript found for agent ID"). The rerun workflow `wf_7492f1c4-d2c` was stopped with 37 results: leads 0-8 verified (0-7 closed: at least two of three verdicts say fixed at head; lead 8 partly fixed, then closed by `8cfc995`: AGENTS.md now describes the repeated DECODE_ATTN threshold waves, checked against `round_robin_cluster_scheduler.py:100-156,300-357`), 7 of 12 finder lenses returned 23 new findings (`rerun_20260924/salvaged_new_findings.json`), `dummy:code` and `scope:S43:vllm` returned. Grok task G3a verified leads 9, 10, 12, 13 and 15-31 (`rerun_20260924/g3a/{verdicts.json,report.md}`; pinned worktree status empty): 18 closed with three of three lenses agreeing; 10, 26 and 31 stayed open as record wording only, confirmed against the files and fixed in the records (lead 10: `design.md` one-lane expectation limited to the 71 original cases and the W3 report corrected; lead 26: `summary.md` and `validation.md` cite `exp-0924-114241-429126`; lead 31: `plan.md` §18 P1(a) marks the oracle comparison as not implemented and assigned to S43); the lead 27 residue in the W6 report is annotated. Spot-checked: `moe_vllm_kernel.py:923`, `test_vllm_dp_placement_runtime.py:293`. Leads 11 and 14 are removed by the W3 refactor. Still to run: verification of the 23 new findings; finders W4:correctness, W7:correctness, W9-05 correctness and quality, CROSS:correctness; `dummy:evidence` and the synthesis; the S43 Frontier part and critique, and all S42 parts. G3b and G3c returned (see "G3b and G3c results" below). The user then invoked `/grok-exec` ("排队的 G3d、G3e、G4依旧交给grok处理"). The three specs were rewritten to the skill template and launched read-only against `.worktrees/rerun-review` moved to `99db5a1`, outputs under each run's `scratch/`: `dummy-mode-evidence-0924a` (G3d), `s43-s42-scope-0924a` (G3e), `normalizer-facts-0924a` (G4); each stops at one checkpoint for lead review. Specs: `/data/ycfeng/tmp/grok_runs/prompts/<label>.md` | `rerun_20260924/salvaged_lead_verdicts_0_8.json`, `/data/ycfeng/tmp/grok_runs/g3a_leads/` |

Stray edit (found 2026-09-24T08:16Z, reverted): `frontier/scheduler/cluster_scheduler/round_robin_cluster_scheduler.py:428` in this worktree carried an uncommitted one-line change of the batch-mode `dp_id` formula. The lead-7 code-truth verifier of `wf_7492f1c4-d2c` (agent `a03a6f813e4c77299`, 07:46:15Z) ran `mkdir && git archive | tar -x -C <export> . && cd <export>; ...; sed -i ...`. The trailing `.` came from the workflow's shared export instruction; it made `tar` fail ("Not found in archive"), so `cd` did not run and the commands after `;` edited this worktree. The Grok prompts use the form without the trailing `.`. The line was restored by the inverse `sed`; `git diff` of the file is empty. No gate read this worktree's source: the gates run on `.worktrees/followup-base` and the candidate trees, and only take scripts from this task directory. Every other worktree was clean apart from G1's expected edits in `.worktrees/sync-waiting-room`. The Grok prompts not yet launched now require exports that fail closed and a clean `git status` of the pinned worktree at the end.

G3b and G3c results (2026-09-24, checked by the lead):

- G3b (`rerun_20260924/g3b/{verdicts.json,report.md}`, pinned worktree status empty) verified the 23 salvaged findings: none refuted, two duplicate pairs (W9-correctness-1 = CROSS-quality-0; HARNESS-correctness-2 = HARNESS-quality-0). The lead re-read the two W9 correctness findings:
  - CROSS-quality-0 holds: `replica_stage_schedule_event.py:90-134` decrements `num_running_batches` on a stale drop and returns without the cluster completion hook, and `vllm_load_balancing_cluster_scheduler.py:103-106` then overwrites `_held_key` on the next full-lane admission. `repro/held/held_key.json`: dense PP4 seed 11 loses key 11 (absent from 399 reports); MoE DP2 EP2 PP4 loses one key; PP2 has no stale drop.
  - W9-correctness-0 holds: at PP>=4 `_get_monolithic_pp_extra_terminal_release_iters` (`vllm_v1_mtp_wait.py:825-836`) is at least 1, and the deferred free (`:905-916`) runs on the no-batch `on_schedule` path (`vllm_v1_engine_replica_scheduler.py:1207-1224`), which returns `None` without a report. G3b's two-request case routes `[0, 1]` at PP4 and `[0, 0]` at PP2.
  - The other 21 findings: the lead read every verdict and its smallest fix, and re-checked three against the code at `99db5a1` (after the W3 refactor):
    - HARNESS-correctness-1 holds. `extract_vllm_placement.py:214` copies the iteration record's `monotonic`, which vLLM writes after `_process_engine_step()` returns (`core.py:1178-1180`, `frontier_trace.py:158`). `admitted_monotonic` is the end of the step, so the S43 admission gaps quoted from it (21.3-47.4 ms, 129.5-311.8 ms) are upper bounds. G3e is told at its checkpoint.
    - CROSS-quality-3 holds: `sync_entry.py:69-70` rejects any mode but the two literals its only callers (`:228`, `:250`) pass.
    - W3-correctness-0 holds: the close path pops only the leaf payload (`sync_entry.py:191-193`); `review.md:396` says rooms are deleted.
    - The rest are accepted on G3b's evidence without a lead re-run.
  - Proposed disposition, pending the user's decision:
    - (A) Core correctness: CROSS-quality-0 (with its duplicate W9-correctness-1, and CROSS-quality-1's oracle assertion plus one PP4 preempting case) and W9-correctness-0.
    - (B) Dead guards and pinning tests: W4-quality-0 (the balancer checks, `time_ms` and 19 cases), CROSS-quality-3, and W4-quality-1..3.
    - (C) Harness `tests/comparison/dp_placement_pp/`: HARNESS-correctness-1 (schedule bound, WG05 rewrite), HARNESS-correctness-3 with HARNESS-quality-3 (mapper for EP-off; reject non-eager), HARNESS-quality-0/1/2/4, and the HARNESS-correctness-0 docstring.
    - (D) Records only: CROSS-quality-2, W3-correctness-0 (`review.md:396`), and W3-correctness-1. The last one is a semantic-alignment row: Frontier follows vLLM's eager side and drops per-lane cuda-graph DP padding in multi-lane cohorts; not measured, not modeled.
    - (E) Tests: W9-04-quality-0/1.
- G3c (`rerun_20260924/g3c/{findings.json,report.md}`) ran five finder lenses and left three findings. The lead checked two of them in full simulations on exports (`followups_20260924/g3c_verify/`, scripts `run.sh`, `run_spec.sh`, `observe_af_victims.py`):
  - CROSS-correctness-0 is reproduced end to end: `examples/architecture/co-location/offline/moe_spec_dec.sh` with `PP=4 NUM_REQUESTS=32 QPS=50 DECODE_TOKENS=64 PREFILL_TOKENS=256` and `--vllm_v1_scheduler_config_num_blocks 40 --vllm_v1_scheduler_config_num_blocks_mode explicit` raises `planned_draft_tokens_per_request length mismatch: expected=1, got=2` on `6d59946`, on `4ef6569`, and on `origin/main` `4ab1964` (also `num_blocks 60`: expected=2, got=3). It predates PR 35. PP2 runs with 45-166 preemptions do not raise: no stage queues a batch, so no live subset is built (observer count 0); PP4 builds one subset with spec metadata before the raise. PP3 is rejected by config (32 layers).
  - W9-05-correctness-0 (an in-flight DECODE_ATTN victim leaves mixed layers and the F-to-A receipt raises) is a real code-level defect in a constructed state, but thirteen PD-AF dense runs on `6d59946` and `4ef6569` did not reach it: 57 DECODE_ATTN preemptions, every victim at layer 0 with no round trip in flight. In those configurations `_schedule_decode_attn_only` is only called with no request in flight, and running never exceeds `micro_batch_size`. The layer reset comes from `c647e95`.
  - W9-05-quality-0 (the pipelined preemption test's overlap check is blind after the epoch advance) is not yet re-read.

Grok task G4 results (2026-09-24, checked by the lead). Label `normalizer-facts-0924a`, spec `/data/ycfeng/tmp/grok_runs/prompts/normalizer-facts-0924a.md`, two turns (Q1-5, then Q6-9 with `normalizer-facts-0924a.turn2.md`), read-only. Outputs: `/data/ycfeng/tmp/grok_runs/normalizer-facts-0924a/scratch/{facts.md,open_decisions.json,probe_q1_q5.py,probe_q1_q5.json}`.

- Acceptance re-run by the lead: `facts.md` has 9 sections; `open_decisions.json` has 16 entries with the required keys; `git -C .worktrees/rerun-review status --short` is empty.
- Spot-checked citations, all matching at `99db5a1`: Frontier TPOT and E2E (`request.py:943-952`, `:318-320`), vLLM TPOT and E2E (`output_processor.py:270-280`), `load_request_metrics` (`tests/e2e/pd_af_parity/harness.py:1130`), `case_request_id` (`extract_vllm_placement.py:67-71`), the `COMPLETE` writer (`run_vllm_worker.sh:50`), the pressure-test digest and hash-mismatch assertions (skill `tests/pressure_scenarios.py:54`, `:327-332`), and the archive path in `tool-boundaries.md:11`.
- Facts the normalizer design depends on:
  - The G5 Frontier runs have no `metrics_ground_truth.jsonl` (the flag was off). The two tested Frontier readers (`load_main_request_metrics`, `validate_request_metrics`) require that sidecar, so the timing join has no input on the existing artifacts.
  - Row classes (warmup, formal, and G3 sizing) exist only on the trace (`request_ids.json`, `client_requests.jsonl`), not on either metric file.
  - No repository function writes normalized rows. The vLLM-side parsers in `tests/comparison/dp_placement_pp/` have no unit tests.
  - The skill pressure test checks only the words `bytes`, `sha256` and `hash mismatch` in `tool-boundaries.md`, so a repository-path pin passes it unchanged unless those words are removed. `migration.md:56-75` and `tool-boundaries.md:17-23` still describe the retired archive.
  - The four helper-owning entries are `e2e-metrics-gap`, `op-supplement`, `cpu-overhead-supplement` and `dispatch-align-trace`.
- The 16 open decisions (clean input, Frontier scheduler CSV, window clocks, copy versus recompute, envelope, non-formal rows, marker and status names, source profiles, Frontier run-check, pin columns, entry text) are for the normalizer design round with the user. None is decided here.

Grok route (2026-09-24): the user asked to delegate heavy engineering work to Grok Build at grok-4.7 xhigh effort and review its results, to limit Opus subagent use. The Grok bridge caps effort at `high`, and a direct Grok CLI launch with `--always-approve` was refused by the permission classifier. The user then adopted a Bash allow rule for `bash /data/ycfeng/tmp/grok_runs/run_grok.sh *` in `~/.claude/settings.json`. The launcher runs `grok --prompt-file <prompt> --cwd <dir> --model grok-4.7 --effort xhigh --always-approve --output-format plain` in the background and writes `output.md`, `stderr.log`, `pid`, `session_id` and `exit` under `/data/ycfeng/tmp/grok_runs/<label>/`. Prompts: `/data/ycfeng/tmp/grok_runs/prompts/`. At most three tasks run at once.
...
- Q1 (a), 2026-09-24: the branch was moved onto `67783c7` and the fix committed as `e2c2937`, with the case `dense_pp4_spec_decode` (dense, PP4, 8 blocks, 12 requests, seed 42) in `tests/integration/test_vllm_v1_decode_preemption_runtime.py`. The case was chosen from a 72-cell probe (`/data/ycfeng/tmp/spec-decode-live-batch/probe/pp4_{red,green}.json`): it raises the length mismatch at batch end on an export of `67783c7` (1 failed, 4 passed) and passes with the fix (5 passed; with the unit file, 35 passed). Gates against `g3bfix` all PASS (`test_report_2026-09-24_spec_decode_live_batch.md`). Branch pushed; draft PR 39 opened with base `fix/issue26-correctness-pr`.
- New finding N1 (pre-existing, not fixed): with speculative decoding and a committed count below `1 + num_speculative_tokens`, the vllm_v1 scheduler frontier (`_scheduled_num_computed_tokens_by_request`, advanced by `_advance_scheduler_num_computed_tokens`, `vllm_v1_kv_allocation.py:112-121`) grows by the verify width every step and is never reduced by the rejected drafts. vLLM subtracts them in `update_from_output` (`.real-engine/vLLM-BS/vllm/v1/core/sched/scheduler.py:1296-1308`). The frontier runs ahead of the request by one token per rejected draft, which inflates KV accounting (`_get_kv_accounted_processed_tokens` takes the larger value) and can strand a request: in the probe's MoE cell (PP1, 64 blocks, 6 requests, seed 5) request 1 ends with frontier 96, processed 89, total 90, and the run stops with non-empty scheduler state. It reproduces at PP1, PP2 and PP4, with no preemption, on `origin/main` `4ab1964`, and does not depend on the token budget or chunked prefill; `committed = k + 1` (no rejection) completes. Evidence: `/data/ycfeng/tmp/spec-decode-live-batch/stall/`. Put to the user.
- N1 decided (a), 2026-09-24 (`requirements.md`): fixed as the second commit of draft PR 39, `fd096ea` "fix(vllm_v1): take rejected drafts off the scheduler frontier at step end". `_roll_back_rejected_drafts` runs in `on_batch_end` after the active marks are released and subtracts each live row's rejected drafts. New `tests/integration/test_vllm_v1_spec_decode_frontier.py` (dense; MoE DP2 EP2): after every speculative step `frontier == tokens - 1` and every request completes; 2 failed on an export of `e2c2937`, 2 passed with the fix. With the fix, the 72-cell PP4 spec probe has all cells complete (`probe/pp4_n1.json`). Gates against `specfix`: suites 0 regressions, examples 16/16, stage matrix all PASS, W9-05 probe 72/72, fidelity 73/74; the one difference, `pdd_spec_dec_offline`, loses a DECODE preemption caused by the frontier running 11 tokens ahead (one extra block), E2E mean 10272.9 -> 9989.2 ms (`test_report_2026-09-24_spec_decode_live_batch.md`). Pushed; PR 39 body (backup `/data/ycfeng/tmp/spec-decode-live-batch/pr39/body_before_n1.md`) and title updated and read back; still draft.

### G3b disposition (D) records and the WG05 rewrite (2026-09-25, completed, lead)

Reason: disposition (D) and HARNESS-correctness-1 left records that state more than the evidence. Method: in-place corrections with a dated note and mod-history rows; no status other than the new rows changes.

- `calibration/dp_pp_case_001/analysis/`: WG05 rewritten from admission intervals `(previous_iteration_monotonic, admitted_monotonic]` (`core.py:385-424`, `:1178-1180`). S43 evidence narrows to a e0 (33.83, 47.35], b e0 (25.31, 31.84], b e1 (25.10, 30.81] and d e0 [104.84, 129.48] ms after the burst's first route; a e1, c e0 and c e1 show no S43 wait. d e1 becomes the new row WG12: step 2 schedules b2's second 20448-token chunk, the whole budget, while the first chunk is in flight, so b4 and b5 wait for step 3 in (129.48, 311.85]; Frontier lane 1 admits b4 at 18 ms. New semantic rows S45 (next prefill chunk scheduled while the previous one is in flight, `scheduler.py:1029-1045` against `vllm_v1_engine_replica_scheduler.py:560-564`; MISMATCH, not reviewed) and S46 (DP padding under CUDA graphs, `gpu_model_runner.py:1904-1927`, `:2001-2002`; MATCH in this eager case, not modeled for `attn_dp>1` with a graph mode; this is W3-correctness-1). Status files: semantic 38 MATCH / 6 MISMATCH; workflow gap 8 MISMATCH over WG02-WG12. CSVs keep CRLF.
- C3 scope stated in `workflow_gap_summary.md`, `validation.md` and `summary.md`: T1 feeds the native reports, so C3 checks the balancer, not Frontier's report emission (HARNESS-correctness-0); that comparison is a step of the S43 task.
- `design.md`: the heartbeat-events claim rests on `_assert_run_conserves_work` (the event-type assertions were removed in `dfb0b25`); the Step 9 ground-truth line points to `validation.md` G3–G5.
- `validation.md`: the fix-review W7 row marked superseded by W7-R4 and the `1b11eff` re-point.
