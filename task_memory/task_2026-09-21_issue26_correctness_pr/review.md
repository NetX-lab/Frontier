...
| Date | Change |
| --- | --- |
| 2026-09-25 | G3b review corrections: the §14.2 W3 waiting-room answer (only leaf payloads are released) and S9-04 (stale drops and deferred terminal releases lost held keys; fixed in `dfb0b25`). |
| 2026-09-24 | Workflow `wf_7606e14e-f10` follow-up: W7 reachability corrected (no Frontier path calls `predict_reduce_scatter`); W7-R4 recorded (a zero cross-server all-reduce hangs in the companion runner; unreachable from Frontier; proposal). W6 native-validation row and the §14.2 negative-control answer corrected (the setup is copied; the W6 control is a slice-restoring mutant). S9-08 qualified (the integration classifier restates the policy's rule). W7-R4 fixed by the user's choice of option (a) (`e922c77`, gitlink `9adf759`). |
| 2026-09-24 | Review of the branch's fixes recorded (`c647e95`..`6aee289`). Corrected: W6 Deviation 1 (no functional-path fallback exists), the W7 delivery (companion `ff11ee6`, cross-server zero payload, Frontier test scope), open item 1 (resolved) and open item 5 (the pass-through flags reach a multi-lane MoE role). |
...
| Deviation 1 from the adaptations above | The `_custom_ops` import went **inside** the low-level `try`, not outside it. Reason: `fused_moe.py` itself imports `_custom_ops` at its top, so the import cannot fail where the low-level API succeeds, and it therefore cannot perturb the two-branch detection. Corrected 2026-09-24: an earlier sentence here claimed that a build lacking `_custom_ops` would select the functional path. It cannot, for the reason just given, and the matching import comment was fixed in `f236c17` (W6-R4). |
...
| Native validation | Test added: `tests/integration/test_moe_fused_expert_numerical_parity.py`, 8 cases, skipping unless CUDA is present and `VLLM_API_VERSION == "0.10.x"`. It drives `_run_fused_moe_iteration` with a copy of the buffer shapes, kernel config and alignment that `profile_fused_moe_kernel` sets up; it does not call that function. Seven cases compare the output tensor against `fused_experts` at `rtol=0, atol=0`, and the FP8 case is a structural check. Corrected 2026-09-24 (workflow `wf_7606e14e-f10` findings 22 and 26): the copy left the profiler's own config and alignment arguments unchecked; `6828581` pins them on the CPU (W6-R6). Neither Torch environment on this host selects the repaired path (vLLM 0.11.0 and 0.28.0), so it runs on an H800 worker under the official `vllm/vllm-openai:v0.10.2` image. Result recorded in `test_report_2026-09-22_w6_fused_expert_arithmetic.md`. |
...
| Item | Outcome |
| --- | --- |
| Companion commit | `fwyc0573/frontier-htsim` branch `fix/zero-payload-input-handling`, `eb7bc4f`, companion **draft** PR 1. Superseded 2026-09-24 by `ff11ee6` on the same branch (row "Cross-server zero payload" below). Three source edits plus a narrowed `.gitignore`; no change to flow generation, topology modelling or latency arithmetic. |
...
| Frontier test scope | The donor test re-drove the runner CLI and the submodule predictor. That is now the companion repository's own coverage, so the Frontier module tests the Frontier boundary instead. Corrected 2026-09-24 (W7-R2): the first version covered one server only, tested `predict_reduce_scatter`, which no Frontier path calls, and a negative-payload guard every backend shares. `6d621c8` replaced it with three `predict_all_to_all` cases: within one server an empty all-to-all keeps its synchronization latency; across two servers (one and two pairwise phases) it costs more than zero and the same as a one-byte payload. |
| Cross-server zero payload (2026-09-24, W7-R1) | `eb7bc4f` accepted a zero payload, but the runner's all-to-all generators were undefined at zero once a pair crossed servers: `pairwise_steps` raised `ZeroDivisionError`, `nccl_pairwise` hung (htsim reads a 0-byte flow as unbounded, `ndp.cpp:934`), and `full_mesh` returned 0 ms. Companion `ff11ee6` fixes the generators; Frontier's gitlink moved to it. Companion 12 passed, Frontier 3 passed; at `eb7bc4f` the new cases fail (`review_20260924/w7_fix/`). |
| Zero payload in the other collectives (2026-09-24, W7-R4) | Not reachable from Frontier. Its all-reduce, all-gather and send/recv payloads come from a real batch with at least one token, idle placeholders are never priced, and a zero MoE-TP all-reduce returns 0.0 through `ZeroPayloadPolicy.EXACT_NOOP`. The companion is still reachable directly: `eb7bc4f` made zero legal for every collective kind, and `ff11ee6` defines zero only for all-to-all. A probe of a cross-server all-reduce (2 x 8 GPUs, DP=16 ring) at payload 0 generated 480 zero-size connections on the cross-server edges (`7->8`, `15->0`). htsim treats such a flow as unbounded, and the run stopped advancing at simulated time 10.88, while the payload-1 run passed 271. At payload 1 there were no zero-size connections (`wf_7606e14e_f10/zero_byte_allreduce_probe.txt`). Before `eb7bc4f` the same input exited 2 as a missing field. Fixed 2026-09-24 by the user's choice of option (a): companion `e922c77` refuses zero for every kind except all-to-all (exit 2, `tensor_bytes=0 is defined only for alltoall, not <kind>`), gitlink `9adf759`. Companion tests 16 passed; the four new cases fail on `ff11ee6` (4 failed, 12 passed). `wf_7606e14e_f10/w7_r4_zero_payload_by_kind.txt`. Inside one server the old runner finished, because intra-server edges are one-byte placeholders; the rule is by kind, so those cases are refused too. |
...
1. RESOLVED 2026-09-24. Whether `_schedule_batch_mode`'s per-replica grouped return order is load-bearing for the consumers of `ClusterScheduleEvent`'s request mapping (W2). It is: `ClusterScheduleEvent` wakes one `ReplicaScheduleEvent` per replica lane in mapping order (`frontier/events/cluster_schedule_event.py:46-71`), and at equal times the event id breaks the tie. `3ec7bbf` keeps the grouping, computes the placement once through `_schedule_lane_round_robin`, and states the reason in the `_schedule_batch_mode` docstring.
...
5. CORRECTED 2026-09-24 (W2-R5). The public MoE examples check `ATTN_TP == MOE_TP * MOE_EP` on the wrapper's own variables, but the flags passed through after that check reach the runtime, which enforces `attn_tp * attn_dp == moe_tp * moe_ep`. A multi-lane MoE role is therefore reachable from a public wrapper, and the fidelity matrix now covers it (`748e757`: co-location, PDD, and a trained PDD row). Whether the release should offer a dedicated wrapper remains a product question.
...
| Question | Finding |
| --- | --- |
| Is every change linked to a demonstrated defect, an essential regression test, or a directly related simplification? | Yes. Each of W2, W3, W4, W6, W7 carries a negative control that fails against the unrepaired source, recorded in `validation.md`. Corrected 2026-09-24 for W6: against the unrepaired source its tests error at fixture setup; the control that fails for the stated reason is a mutant restoring only the slice arithmetic (`validation.md` Step 6). W5 produced no source change. The only additions without a paired defect are `tests/frontier_sources.py`, which exists because the gitlink bump exposed a real scan defect, and the `AGENTS.md` scheduler list, which documents `VllmLoadBalancingClusterScheduler` added by W4. |
| Are state ownership and initialization explicit? Are repeated fallback checks or parallel state representations still present without a reason? | W3's waiting rooms are owned by one structure keyed by step id, initialized at creation and deleted at release; the tests assert no dangling room survives a cohort. No fallback chain was added. **Corrected 2026-09-25 (G3b review):** release deletes only the leaf payloads. The emptied step, layer and pre-MoE containers remain (16 and 180 empty leaves in two drained MoE runs). No result reads them. |
...
| S9-04 | correctness of the single held slot | One `_held_key` per lane suffices. An admission is held only when it fills the pipeline (`running == PP`); the loop admits only while `running < PP` (`base_replica_scheduler.py:1052`); and `GlobalBatchEndEvent` calls `on_batch_end` (decrement) and then the completion hook in the same handler, before any later `ReplicaScheduleEvent` can admit again. So a held key is always consumed before the next held admission on that lane. The unit case "a completion plus the admission it makes room for" pins that order. **Corrected 2026-09-25 (G3b review):** the argument missed two paths. A batch dropped as stale at a later stage decremented `running` without the completion hook, so the next held admission overwrote the key (dense PP4 seed 11 lost key 11). At PP≥4 a deferred terminal release in an iteration that schedules nothing published no report. Fixed in `dfb0b25`; the integration oracle `_assert_every_held_key_is_reported` checks every held key. | Accepted, no guard added; corrected by `dfb0b25` |
...
| S9-08 | test surface | The tests stay out of `frontier/` (R9-07). Each new or changed unit case fails on the pre-P2 tree (19 of 19). The integration test checks each report against the reference iteration kind, not against Frontier's own output, and its discriminating case separates the policy from a completion-reporting control. Qualified 2026-09-24 (workflow `wf_7606e14e-f10` finding 31): its classifier `running_after >= num_pipeline_stages` restates the policy's own room rule (`lane.num_running_batches < self._num_pipeline_stages`), so a change applied to both passes it; the reference oracle is not compared with Frontier's reports. Comparing them is a step of the S43 task. | Accepted |
