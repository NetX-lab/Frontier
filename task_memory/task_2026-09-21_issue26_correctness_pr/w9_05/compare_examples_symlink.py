# W9-05 B8: compare the 16 example outputs; trees/before is a symlink, so both its literal and resolved paths are substituted.
import sys
from pathlib import Path
P = Path("/data/ycfeng/tmp/issue26-correctness-pr/w9_05")
...
for name in names:
    subs = {side: [PathSubstitution(str((P / "examples" / side / name).resolve()), "<CASE>"),
                   PathSubstitution(str((P / "trees" / side).resolve()), "<TREE>"), PathSubstitution(str(P / "trees" / side), "<TREE>")] for side in ("before", "after")}
