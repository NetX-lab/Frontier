...
from frontier.profiling.cpu_overhead.schema import DEFAULT_SCHEDULING_MODE
...
try:
    ...
    LLMEngine = None
    SamplingParams = None
    CpuOperationMetrics = None
    _SARATHI_IMPORT_ERROR = exc
else:
    _SARATHI_IMPORT_ERROR = None
...
def _resolve_engine_dtype(precision: str) -> str:
    precision_type = PrecisionType.from_string(precision)
    if precision_type == PrecisionType.FP16:
        return "float16"
    if precision_type == PrecisionType.BF16:
        return "bfloat16"
    if precision_type == PrecisionType.FP32:
        return "float32"
    raise ValueError(
        "Unsupported precision for CPU overhead profiling: "
        f"{precision_type.name}. Supported values: FP32, FP16, BF16."
    )
...
class BenchmarkRunner:
    def __init__(
        ...
        if _SARATHI_IMPORT_ERROR is not None:
            raise RuntimeError(
                "CPU overhead profiling requires 'sarathi' runtime dependencies. "
                "Please install and configure sarathi before running this profiler."
            ) from _SARATHI_IMPORT_ERROR

        self._model_name = model_name
        self._batch_size = batch_size
        self._tensor_parallel_degree = tensor_parallel_degree
        self._output_dir = output_dir
        self._precision = PrecisionType.from_string(precision).name
        ...
        self._llm_engine = LLMEngine.from_engine_args(
            ...
            dtype=_resolve_engine_dtype(self._precision),
            load_format="dummy",
            # scheduler config
            scheduler_type="vllm",
        ...
        metrics.update(
            {
                ...
                "num_prefill_tokens": NUM_PREFILL_TOKEN,
                "num_decode_tokens": self._batch_size
                * NUM_DECODE_TOKEN_AMPLIFICATION_FACTOR,
                "scheduling_mode": DEFAULT_SCHEDULING_MODE,
                "ray_comm_time_mean": ray_comm_time_mean,
                "profiling_precision": self._precision,
                "cpu_overhead_source": "measured",
            }
        )
