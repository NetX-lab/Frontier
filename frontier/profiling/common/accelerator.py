...
def warn_if_platform_mismatch(
    gpu_platform: str,
    backend_platform: str | None,
) -> bool:
    """Warn about a device/backend platform mismatch without changing either choice."""

    device_platform = str(gpu_platform).strip().lower()
    selected_backend = (
        None if backend_platform is None else str(backend_platform).strip().lower()
    )
    if selected_backend is None or device_platform == selected_backend:
        return False
    warnings.warn(
        "Configured GPU platform %r does not match backend platform %r; "
        "preserving the user-selected backend." % (device_platform, selected_backend),
        RuntimeWarning,
        stacklevel=2,
    )
    return True
