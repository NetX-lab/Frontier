...
class CudaTimer:
    ...
    def handle_trace(self, trace):
        ...
        # Prefer newer device_time_total to avoid deprecation warnings; fall back for older PyTorch
        if hasattr(events[0], "device_time_total"):
            total_device_time = self.aggregation_fn([e.device_time_total for e in events])
        else:
            total_device_time = self.aggregation_fn([e.cuda_time_total for e in events])

        self.timer_stats_store.record_time(
            self.name, total_device_time * 1e-3
        )  # convert to ms
