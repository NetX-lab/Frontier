...
@dataclass
class SJ2QPenaltyOnlySessionState:
    session_id: int
...
class SJ2QPenaltyOnlyReplicaScheduler(VLLMv1EngineReplicaScheduler):
    """Two-queue penalty-only scheduler without long-history rescue."""

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._sj2q_penalty_session_states: Dict[int, SJ2QPenaltyOnlySessionState] = {}
        self._sj2q_penalty_long_round_threshold = int(
            getattr(self._config, "long_round_new_prompt_threshold", 4096)
        )
        self._sj2q_penalty_service_cap_tokens = int(
            getattr(self._config, "service_cap_tokens", 8192)
        )
        self._sj2q_penalty_long_liveness_quota = int(
            getattr(self._config, "long_liveness_quota", 32)
        )
        self._sj2q_penalty_short_streak_counter = 0
        self._sj2q_penalty_forced_qlong_request_id: Optional[int] = None
    ...
    def _get_session_id(self, request: Request) -> int:
        ...
        if session_id is None:
            raise ValueError(
                "SJ2QPenaltyOnlyReplicaScheduler requires request.session_id."
            )
    ...
    def _get_request_state_snapshot(self, request: Request) -> Optional[dict]:
        snapshot = getattr(request, "_sj2q_penalty_state_snapshot", None)
        if snapshot is None:
            return None
        if not isinstance(snapshot, dict):
            raise TypeError(
                "request._sj2q_penalty_state_snapshot must be a dict when present."
            )
        return snapshot

    def _load_or_create_session_state(self, request: Request) -> SJ2QPenaltyOnlySessionState:
        session_id = self._get_session_id(request)
        existing_state = self._sj2q_penalty_session_states.get(session_id)
        if existing_state is not None:
            return existing_state

        snapshot = self._get_request_state_snapshot(request)
        if snapshot is not None:
            state = SJ2QPenaltyOnlySessionState(
            ...
            state = SJ2QPenaltyOnlySessionState(session_id=session_id)

        self._sj2q_penalty_session_states[session_id] = state
        return state
    ...
    def _sync_request_state_snapshot(
        ...
        setattr(request, "_sj2q_penalty_state_snapshot", asdict(state))
        setattr(request, "_sj2q_penalty_queue_level", state.queue_level)
        setattr(request, "_sj2q_penalty_qshort_entries_total", state.qshort_entry_count)
        setattr(
            request,
            "_sj2q_penalty_qshort_long_history_entries_total",
            state.qshort_long_history_entry_count,
        )
        setattr(request, "_sj2q_penalty_qlong_entries_total", state.qlong_entry_count)
        setattr(
            request,
    ...
    def _classify_new_round(
        ...
        if state.long_history:
            queue_level = QLONG
        elif state.served_new_tokens_total > self._sj2q_penalty_service_cap_tokens:
            self._mark_long_history(request, state)
            queue_level = QLONG
        elif round_new_prompt_tokens > self._sj2q_penalty_long_round_threshold:
            self._mark_long_history(request, state)
            queue_level = QLONG
        else:
            queue_level = QSHORT
    ...
    def _get_state_for_request(
        self,
        request: Request,
    ) -> Optional[SJ2QPenaltyOnlySessionState]:
        try:
            session_id = self._get_session_id(request)
        except ValueError:
            return None
        return self._sj2q_penalty_session_states.get(session_id)
    ...
    def _queue_rank(self, request: Request) -> int:
        ...
        if self._sj2q_penalty_forced_qlong_request_id == request_id:
            return -1
    ...
    def _sort_requests_by_queue_level(
        ...
        self._sj2q_penalty_forced_qlong_request_id = self._select_forced_qlong_request_id(
            requests
        )
    ...
    def _account_service_tokens(
        ...
        *,
        ...
        if (
            state.served_new_tokens_total > self._sj2q_penalty_service_cap_tokens
    ...
    def _apply_long_liveness(self, scheduled_levels: Sequence[str]) -> None:
        if any(level == QLONG for level in scheduled_levels):
            self._sj2q_penalty_short_streak_counter = 0
            self._sj2q_penalty_forced_qlong_request_id = None
            return
        self._sj2q_penalty_short_streak_counter += sum(
