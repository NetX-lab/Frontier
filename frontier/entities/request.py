...
def validate_inter_cluster_transfer_request_cohort(
    requests: Sequence["Request"],
) -> None:
    """Validate one M2N Request cohort without mutating any Request."""
    if not requests:
        raise ValueError("M2N transfer request cohort must not be empty")

    request_ids = [request.id for request in requests]
    if len(set(request_ids)) != len(request_ids):
        raise ValueError(
            "M2N transfer request cohort contains duplicate request IDs: "
            f"{request_ids}"
        )
...
class Request(BaseEntity):
    def __init__(
        ...
        # In PD+AF mode, the prefill-to-decode handoff already exposes the first
        # output token while request-level processed-token accounting remains
        # anchored at the prompt length.  Keep this observation separate from
        # processed tokens so the decode scheduler executes exactly N-1 real
        # decode stacks for N output tokens.
        self._num_handoff_emitted_decode_tokens: int = 0
    ...
    @property
    def num_emitted_decode_tokens(self) -> int:
        """Return output tokens observed by the user, including handoff output."""
        return min(
            self.num_processed_decode_tokens + self._num_handoff_emitted_decode_tokens,
    ...
    def remaining_decode_tokens(self) -> int:
        return max(self._num_decode_tokens - self.num_emitted_decode_tokens, 0)

    def on_disaggregated_decode_handoff(
        self,
        time: float,
        cluster_type: ClusterType,
    ) -> None:
        """Record the first output token exposed at a PD+AF decode handoff."""
        if cluster_type not in (ClusterType.DECODE, ClusterType.DECODE_ATTN):
            raise ValueError(
                "Disaggregated decode handoff is only valid for DECODE/DECODE_ATTN, "
                f"got {cluster_type.name}"
            )
        if not self._is_prefill_complete:
            raise ValueError(
                f"Cannot hand off request {self._id} to {cluster_type.name} "
                "before prefill completes"
            )
        if self._num_decode_tokens <= 1 or self.num_emitted_decode_tokens >= 1:
            return

        self._completed_layer_count = 0
        self._af_roundtrip_inflight = False
        self._num_handoff_emitted_decode_tokens = 1
        if self._first_decode_token_completed_at == 0:
            self._first_decode_token_completed_at = (
                self._prefill_completed_at if self._prefill_completed_at > 0 else time
            )
    ...
    def on_batch_end(
        ...
        decode_tokens_before = self.num_emitted_decode_tokens
        ...
        decode_tokens_after = self.num_emitted_decode_tokens
        if self._first_decode_token_completed_at == 0:
        ...
        if (
            decode_tokens_after >= self._num_decode_tokens
            or self._num_processed_tokens == self.total_tokens
        ):
    ...
    def validate_inter_cluster_transfer_start(
        self,
        time: float,
        ...
        """Validate transfer-start lifecycle state without mutating the request."""
        if (
            source_cluster == ClusterType.DECODE_ATTN
            and target_cluster == ClusterType.DECODE_FFN
            and self._af_roundtrip_inflight
        ):
            raise ValueError(
    ...
    def on_inter_cluster_transfer_start(
        ...
        residence tracking.
        """
        self.validate_inter_cluster_transfer_start(
            time=time,
    ...
    def validate_inter_cluster_transfer_end(
        self,
        time: float,
        source_cluster: ClusterType,
        target_cluster: ClusterType,
        ...
        """Validate transfer-end lifecycle state without mutating the request."""
        if (
            source_cluster == ClusterType.DECODE_ATTN
            and target_cluster == ClusterType.DECODE_FFN
        ):
        ...
        if (
            source_cluster == ClusterType.DECODE_FFN
            and target_cluster == ClusterType.DECODE_ATTN
            and not self._af_roundtrip_inflight
        ):
            raise ValueError(
    ...
    def on_inter_cluster_transfer_end(
        ...
        self.validate_inter_cluster_transfer_end(
            time=time,
            source_cluster=source_cluster,
            target_cluster=target_cluster,
            activation_size_bytes=activation_size_bytes,
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "num_handoff_emitted_decode_tokens": self._num_handoff_emitted_decode_tokens,
    ...
    def restart(self):
        ...
        self._num_handoff_emitted_decode_tokens = 0
