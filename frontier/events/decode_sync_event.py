...
class DecodeSyncEvent(BaseEvent):
    """Event to handle MoE decode synchronization points for a target cluster."""
    ...
    def __init__(
        ...
        super().__init__(time, EventType.DECODE_SYNC)
    ...
    def handle_event(
        ...
        request_ids = [req.id for req in self._batch.requests] if self._batch.requests else []
        logger.info(
        ...
        return cluster_scheduler.on_decode_sync(
            self.time,
            self._replica_id,
            self._stage_id,
            self._batch,
