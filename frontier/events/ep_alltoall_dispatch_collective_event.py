...
class EPAllToAllDispatchCollectiveEvent(BaseEvent):
    """Event emitted when EP dispatch collective completes for one logical batch."""

    def __init__(
        self, time: float, replica_id: int, stage_id: int, batch_global_id: int
    ) -> None:
        super().__init__(time, EventType.EP_ALLTOALL_DISPATCH_COLLECTIVE)
    ...
    def handle_event(
        ...
        return cluster_scheduler.on_ep_alltoall_dispatch_collective_schedule(
            self.time, self._replica_id, self._stage_id, self._batch_global_id
        )
