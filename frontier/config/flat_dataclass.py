...
from frontier.config.utils import (
    get_all_subclasses,
    get_inner_type,
    is_bool,
...
def _resolve_string_annotation(annotation: str, dataclass_cls: Any) -> Any:
    """
    Resolve a string type annotation to the actual type.
    
    This is needed when using `from __future__ import annotations` which
    causes all annotations to be stored as strings (PEP 563).
    
    Args:
        annotation: The string annotation to resolve
        dataclass_cls: The dataclass class containing the annotation
    ...
    # Special handling for CC Backend config classes to avoid circular imports
    # These classes are defined in frontier.cc_backend.cc_backend_config but
    # referenced in frontier.config.config, which creates a circular import.
    cc_backend_config_classes = {
        'BaseCCBackendConfig',
    ...
    if annotation in cc_backend_config_classes:
        try:
            ...
            return {
                'BaseCCBackendConfig': BaseCCBackendConfig,
                'VidurCCBackendConfig': VidurCCBackendConfig,
                'AnalyticalCCBackendConfig': AnalyticalCCBackendConfig,
                'CollectiveSimCCBackendConfig': CollectiveSimCCBackendConfig,
                'AstraSimAnalyticalCCBackendConfig': AstraSimAnalyticalCCBackendConfig,
            }[annotation]
        except ImportError:
            pass
    
    # Try to get type hints which will resolve forward references
    try:
        # Get the module where the dataclass is defined
        module = dataclass_cls.__module__
        import sys
        module_obj = sys.modules.get(module)
        if module_obj:
            globalns = getattr(module_obj, '__dict__', {})
            # Try to evaluate the annotation in the module's namespace
            try:
    ...
    # If we can't resolve it, return the original string
    return annotation
...
def create_flat_dataclass(input_dataclass: Any) -> Any:
    ...
    def process_dataclass(_input_dataclass, prefix=""):
        ...
        for field in fields(_input_dataclass):
            if field.metadata.get("include_in_cli", True) is False:
                continue

            prefixed_name = f"{prefix}{field.name}"

            # Resolve string annotations to actual types (needed for PEP 563)
            raw_field_type = field.type
            if isinstance(raw_field_type, str):
                raw_field_type = _resolve_string_annotation(raw_field_type, _input_dataclass)

            if is_optional(raw_field_type):
                field_type = get_inner_type(raw_field_type)
            else:
                field_type = raw_field_type

            # Resolve string annotations again after unwrapping Optional
            if isinstance(field_type, str):
                field_type = _resolve_string_annotation(field_type, _input_dataclass)
            ...
            if is_subclass(field_type, BasePolyConfig):
                ...
                for subclass in get_all_subclasses(field_type):
                    if getattr(subclass, "__include_in_cli__", True) is False:
                        continue
