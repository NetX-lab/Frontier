...
class BaseFixedConfig(ABC):
    ...
    def create_from_name(cls, name: str) -> Any:
        for subclass in get_all_subclasses(cls):
            # Use get_static_name() for BaseModelConfig subclasses, get_name() for others
            if hasattr(subclass, 'get_static_name'):
                if subclass.get_static_name() == name:
                    return subclass()
            elif subclass.get_name() == name:
                return subclass()
