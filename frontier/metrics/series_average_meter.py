...
class SeriesAverageMeter:
    def __init__(
        ...
        self._store_data_series = store_data_series

        self._data_series = [] if self._store_data_series else None
    ...
    def print_stats(self, name: str, path: str) -> None:
        if self._denom_sum == 0:
            if not self._data_series:
                return
            weighted_mean = (
                self._last_data_y if self._last_data_y is not None else 0.0
            )
        else:
        ...
        if self._store_data_series:
            stats_dict["time_series"] = [
                {"time": x, "value": y} for x, y in self._data_series
            ]
