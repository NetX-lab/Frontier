...
|------------|--------------------|
| 2026-07-23 | Added the complete sequential PD-AF `pre-release-v0.3` example surface and clarified its supported and deferred features. |
...
📍[2026/07] Prefill-Decode Disaggregation (PDD) and sequential Attention-FFN Disaggregation (PD-AF) support is available in the v0.3 development release.<br />
...
> PD-AF is exposed through the sequential `pd-af-disaggregation` path with dense, MoE, EP, and global CUDA Graph examples. Parallel PD-AF execution remains guarded. Thinking Mode, Speculative Decoding / MTP, and Prefix Caching remain outside the PD-AF v0.3 surface.
...
Current release-facing PDD and co-location examples, plus the PD-AF examples, are split by simulation mode and default to the formula-based `analytical` backend for one-click smoke runs:
...
The PDD and co-location examples cover most runtime optimizations. The PD-AF `run_all.sh` entrypoint covers five offline and five online dense/MoE/EP/CUDA Graph cases. Dummy analytical smoke runs validate runtime plumbing, not profiling fidelity; dummy-mode evidence is not trained numerical parity. Profiling outputs and reusable compute data are organized under `data/profiling/compute`.
