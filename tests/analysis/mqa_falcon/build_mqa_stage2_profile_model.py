#!/usr/bin/env python3
"""Build Falcon MQA Stage 2 profiling and error-matrix artifacts."""
...
class FalconMqaStage2Config:
    """Topology and runtime constants for the Falcon MQA Stage 2 probe."""
    ...
    model_arch: str = "falcon_mqa"
    quant_signature: str = "dense_bf16"
    attention_backend: str = "FLASHINFER_VLLM_V1"
    flashinfer_python_expected_version: str = "0.3.1.post1"
    synthetic_tp_mock: bool = False
...
def summarize_attention_scopes(rows: list[dict[str, Any]]) -> dict[str, ScopeSummary]:
    ...
    for scope in REQUIRED_SCOPES:
        ...
        summaries[scope] = ScopeSummary(
            ...
            flashinfer_window_left=int(first_meta["flashinfer_window_left"]),
    ...
    output_row.update(
        {
            ...
            "synthetic_tp_mock": config.synthetic_tp_mock,
        }
    )
    return output_row
...
def local_q_heads(
    ...
    *,
    ...
    if num_q_heads % tp_size != 0:
        if native_falcon:
            raise ValueError(
                "native Falcon-7B Q heads are not divisible by requested TP: "
                f"num_q_heads={num_q_heads}, tp_size={tp_size}"
            )
        raise ValueError(
            "Q heads must divide TP size: "
            f"num_q_heads={num_q_heads}, tp_size={tp_size}"
        )
    return num_q_heads // tp_size
...
def local_kv_heads(num_kv_heads: int, tp_size: int) -> int:
    ...
    if num_kv_heads >= tp_size:
        if num_kv_heads % tp_size != 0:
            raise ValueError(
                "KV heads must divide TP size for partitioned KV heads: "
...
def _page_bytes(config: FalconMqaStage2Config, tp_size: int) -> int:
    local_q_heads(
        config.num_q_heads,
        tp_size,
        native_falcon=not config.synthetic_tp_mock,
    )
    return (
        config.bytes_per_element
        * 2
        * config.block_size
        * local_kv_heads(config.num_kv_heads, tp_size)
        * config.head_dim
    )


def compute_mqa_memory_metrics(
    *,
    config: FalconMqaStage2Config,
    observed_max_context_tokens: int,
) -> dict[str, int]:
    """Compute native Falcon MQA page bytes and observed context bytes."""
    ...
    native_tp1_page_bytes = _page_bytes(config, config.tp_size)
    observed_blocks = math.ceil(observed_max_context_tokens / config.block_size)
    return {
        "native_tp1_page_bytes_per_layer": native_tp1_page_bytes,
        "observed_context_tokens": observed_max_context_tokens,
        "observed_context_blocks": observed_blocks,
        "observed_context_bytes_per_layer_tp1": observed_blocks * native_tp1_page_bytes,
    }
...
def build_error_matrix(
    ...
    """Build latency and memory error matrix for transformed live MQA rows."""

    del frontier_attention_rows
    ...
    memory = compute_mqa_memory_metrics(
        config=config,
...
def _ensure_flashinfer_version(config: FalconMqaStage2Config) -> str:
    try:
...
def main() -> int:
    ...
    parser.add_argument(
        "--cuda-op-log",
        type=Path,
        default=Path("/tmp/frontier_mqa_falcon_flashinfer_live_probe/cuda_ops.jsonl"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/tmp/frontier_mqa_falcon_stage2_profile_model"),
    )
    ...
    try:
        summary = write_stage2_outputs(
            args.cuda_op_log,
            args.output_dir,
            FalconMqaStage2Config(),
        )
    except Exception as exc:
        print(str(exc), file=sys.stderr)
        return 1

    print("MQA Stage 2 profile/model artifacts written")
