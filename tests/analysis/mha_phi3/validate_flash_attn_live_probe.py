#!/usr/bin/env python3
"""Validate Phi-3 MHA FlashAttention live-probe CUDA op output."""
    ...
    for scope in REQUIRED_SCOPES:
        for row in by_scope[scope]:
            ...
            if int(meta["head_dim"]) != expected_head_dim:
                raise ValueError(
                    f"Unexpected head_dim for {scope}: {meta['head_dim']!r}"
                )
...
def _print_summary(by_scope: dict[str, list[dict[str, Any]]]) -> None:
    print("FlashAttention live probe validation passed.")
    for scope in REQUIRED_SCOPES:
        rows = by_scope[scope]
...
def main() -> int:
    ...
    parser.add_argument("--expected-backend", type=str, default="FLASH_ATTN_VLLM_V1")
    parser.add_argument("--expected-head-dim", type=int, default=96)
    ...
    try:
        ...
        _validate_row_values(by_scope, args.expected_backend, args.expected_head_dim)
        _require_run_log(
            args.run_log,
            (
