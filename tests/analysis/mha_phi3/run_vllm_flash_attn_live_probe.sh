...
OUTPUT_DIR="${OUTPUT_DIR:-/tmp/frontier_mha_phi3_flash_attn_live_probe}"
...
EXPECTED_BACKEND="${EXPECTED_BACKEND:-FLASH_ATTN_VLLM_V1}"
...
if [[ "$VLLM_ATTENTION_BACKEND" != "FLASH_ATTN" ]]; then
  echo "ERROR: Phi-3 MHA CR-005 requires VLLM_ATTENTION_BACKEND=FLASH_ATTN to select the V1 FlashAttention backend. Got VLLM_ATTENTION_BACKEND=${VLLM_ATTENTION_BACKEND}." >&2
  exit 2
fi
...
"$PYTHON_BIN" "$PROJECT_ROOT/tests/analysis/mha_phi3/validate_flash_attn_live_probe.py" \
  --cuda-op-log "$CUDA_EVENT_OP_LOG_PATH" \
  --run-log "$RUN_LOG" \
  --expected-backend "$EXPECTED_BACKEND" \
  --expected-head-dim 96 \
  --expected-tp "$TP_SIZE" \
  --expected-chunked-prefill-enabled "$CHUNKED_PREFILL_FLAG" \
  | tee "$VALIDATION_LOG"

printf 'MHA Phi-3 FlashAttention live probe artifacts:\n'
printf '  output_dir=%s\n' "$OUTPUT_DIR"
printf '  cuda_ops=%s\n' "$CUDA_EVENT_OP_LOG_PATH"
printf '  run_log=%s\n' "$RUN_LOG"
