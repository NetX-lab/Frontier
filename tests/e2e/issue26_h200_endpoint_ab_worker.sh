#!/usr/bin/env bash
# Compare the endpoint probe against its disabled baseline on the same H200 case.
set -euo pipefail
PREFIX_MODE="${ISSUE26_PREFIX_CACHING:?Set the user-approved prefix caching policy.}"
case "$PREFIX_MODE" in
  disabled) PREFIX_FLAG=--no-enable-prefix-caching ;;
  enabled) PREFIX_FLAG=--enable-prefix-caching ;;
  *) echo "Invalid prefix caching policy: $PREFIX_MODE" >&2; exit 1 ;;
esac
...
for MODE in baseline endpoints; do
  ...
  "$PY" -m vllm.entrypoints.cli.main serve /data/ycfeng/tmp/issue26-qwen3-dummy \
    ...
    --gpu-memory-utilization 0.9 --no-enable-chunked-prefill "$PREFIX_FLAG" \
...
echo ENDPOINT_AB_EXECUTION_COMPLETE
