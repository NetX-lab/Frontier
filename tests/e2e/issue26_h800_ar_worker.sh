#!/usr/bin/env bash
# Collect bounded post-MoE AR evidence for the frozen H800 case.
set -euo pipefail
...
source /data/ycfeng/tmp/issue26_h800_environment_probe.sh "${1:?Provide a fresh output directory.}"
...
export VLLM_FRONTIER_DIAG_MOE_AR_MODE="${3:-normal}"
...
if [[ "$DIAGNOSTIC_SELECTION" == communication ]]; then
  if [[ "${VLLM_FRONTIER_DIAG_MOE_AR_MODE}" == scalar_sync ]]; then
  export VLLM_FRONTIER_CUDA_EVENT_OP_SCOPES=expert_parallel_allreduce_diag_scalar,attn_post_proj_tp_allreduce,tensor_parallel_allreduce
else
...
for MODE in "${MODES[@]}"; do
  RUN="$PROBE_ROOT/$MODE"
  mkdir "$RUN"
  export VLLM_FRONTIER_MOE_BOUNDARY_LOG_PATH="$RUN/moe_boundary"
...
Path(sys.argv[1]).write_text(json.dumps({
    ...
    "python": sys.version, "warmup_rounds": int(os.environ.get("ISSUE26_WARMUPS", "3")), "formal_requests": 1,
...
PYMETA
  "$PY" -m vllm.entrypoints.cli.main serve /data/ycfeng/tmp/issue26-qwen3-dummy \
    ...
    --gpu-memory-utilization 0.9 --block-size 16 --num-gpu-blocks-override 176000 \
...
PY
  "$PY" "$REPO_ROOT/tests/e2e/issue26_token_id_client.py" \
    ...
    --requests 1 --warmups "${ISSUE26_WARMUPS:-3}" --qps 2 --seed 20260908 \
