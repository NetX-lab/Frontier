#!/usr/bin/env bash
# Capture one first-formal PPLX model-forward CUDA boundary on H200.
set -euo pipefail
...
export VLLM_FRONTIER_INSTRUMENTATION=1 VLLM_FRONTIER_TRACE_SKIP_WARMUP=1
...
export VLLM_FRONTIER_BATCH_BOUNDARY_LOG_PATH="$RUN/server.boundary.jsonl"
export VLLM_FRONTIER_BATCH_BOUNDARY_REQUEST_PREFIX="cmpl-pf4096_dc1024:0-0"

SOURCE="${ISSUE26_DIAGNOSTIC_VLLM_SOURCE:-/data/ycfeng/tmp/issue26-vllm-pplx-boundary-20260913-wt}"
COMMIT="${ISSUE26_DIAGNOSTIC_VLLM_COMMIT:-f025cc30a9b8e59fe0304e7ce8b03c76a2fb5c8e}"
...
Path(sys.argv[1]).write_text(json.dumps({
    ...
    "boundary_request_prefix": os.environ[
        "VLLM_FRONTIER_BATCH_BOUNDARY_REQUEST_PREFIX"],
    "instrumentation": {
        "batch_boundary": True,
        "per_op": False,
        "scheduler": False,
    ...
    "limits": (
        "One first-formal model-forward CUDA event envelope per DP0 TP rank; "
        "one post-end synchronization only to read elapsed_time."
    ),
}, indent=2) + "\n")
PYMETA
