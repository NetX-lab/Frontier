#!/bin/bash
# =============================================================================
# PD-AF / pd-af-disaggregation Offline Mode - MoE Model EP Example
# =============================================================================
# This script demonstrates the pd-af-disaggregation architecture with expert
# parallelism (EP) across the MoE clusters.
#
# Architecture:
#   Prefill cluster  -> processes prefill phase (attention + MoE together)
#   Decode-Attn cluster -> processes decode attention phase
#   Decode-FFN cluster  -> processes decode MoE/FFN phase (with EP)
#
...
DECODE_ATTN_MICRO_BATCH_SIZE="${DECODE_ATTN_MICRO_BATCH_SIZE:-1}"
...
# --- Decode-FFN cluster parallelism (EP structure) ---
DECODE_FFN_PP="${DECODE_FFN_PP:-1}"
DECODE_FFN_MOE_TP="${DECODE_FFN_MOE_TP:-1}"
DECODE_FFN_MOE_EP="${DECODE_FFN_MOE_EP:-2}"
...
# --- Metrics ---
METRICS_OUTPUT_DIR="${METRICS_OUTPUT_DIR:-$REPO_ROOT/outputs/examples/pd-af-disagg/offline}"
RUN_ID="${RUN_ID:-moe_model_ep}"
...
if (( PREFILL_MOE_EP > TOTAL_EXPERTS )); then
  echo "ERROR: PREFILL_MOE_EP ($PREFILL_MOE_EP) must be <= TOTAL_EXPERTS ($TOTAL_EXPERTS)" >&2
  exit 2
fi

if (( DECODE_FFN_MOE_EP > TOTAL_EXPERTS )); then
  echo "ERROR: DECODE_FFN_MOE_EP ($DECODE_FFN_MOE_EP) must be <= TOTAL_EXPERTS ($TOTAL_EXPERTS)" >&2
  exit 2
fi
...
cat <<EOF
=========================================================
  PD-AF / pd-af-disaggregation Mode - MoE EP Example
=========================================================
