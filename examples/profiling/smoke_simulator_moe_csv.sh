#!/bin/bash
# =============================================================================
# Profiling Example - MoE CSV Downstream Simulator Smoke
# =============================================================================
# Feeds checked-in linear_op.csv, attention.csv, and moe.csv profiles directly
# into the simulator without dummy predictor mode.
#
...
RUN_ID="${RUN_ID:-profiling_moe_csv_smoke}"
...
MOE_CSV="${MOE_CSV:-$DATA_DIR_BASE/compute/$DEVICE/$MODEL/moe.csv}"
...
for csv_path in "$LINEAR_OP_CSV" "$ATTENTION_CSV" "$MOE_CSV"; do
  if [ ! -f "$csv_path" ]; then
...
CMD=(
  ...
  --random_forrest_execution_time_predictor_config_moe_input_file "$MOE_CSV"
...
cat <<EOF
============================================
  MoE CSV Downstream Simulator Smoke
============================================
